 ...
 * license information.
 */
#ifndef LIGHTGBM_OBJECTIVE_RANK_CALIBRATION_OBJECTIVE_HPP_
#define LIGHTGBM_OBJECTIVE_RANK_CALIBRATION_OBJECTIVE_HPP_
 ...
 */
class RankingCalibrationObjective : public ObjectiveFunction {
 public:
  explicit RankingCalibrationObjective(const Config& config)
      : seed_(config.objective_seed),
        rank_weight_(config.rank_weight) {}

  explicit RankingCalibrationObjective(const std::vector<std::string>&) : seed_(0) {}

  ~RankingCalibrationObjective() {}
...
#pragma omp parallel for schedule(guided)
 ...
 protected:
  ...
  //ranking weight for the rank calibration objective
  double rank_weight_;
};
 ...
 */
class LambdarankCalibrationNDCG : public RankingCalibrationObjective {
 public:
  explicit LambdarankCalibrationNDCG(const Config& config)
      : RankingCalibrationObjective(config),
  ...
  explicit LambdarankCalibrationNDCG(const std::vector<std::string>& strs)
      : RankingCalibrationObjective(strs) {}

  ~LambdarankCalibrationNDCG() {}

  void Init(const Metadata& metadata, data_size_t num_data) override {
    RankingCalibrationObjective::Init(metadata, num_data);
...
#pragma omp parallel for schedule(static)
  ...
  inline void GetGradientsForOneQuery(data_size_t query_id, data_size_t cnt,
    ...
    //  const double curr_score = score[i];
    //  const double true_label = static_cast<double>(label[i]);
    //  Log::Info("score[%d]: %f", i, curr_score);
    //  Log::Info("true_label[%d]: %f", i, true_label);
    //  Log::Info("lambdas[%d]: %f", i, lambdas[i]);
    //  Log::Info("hessians[%d]: %f\n", i, hessians[i]);
    //}
  }
  ...
  const char* GetName() const override { return "multi_rank_calibration"; }
 ...
 */
class RankCalibrationXENDCG : public RankingCalibrationObjective {
 public:
  explicit RankCalibrationXENDCG(const Config& config) : RankingCalibrationObjective(config) {}

  explicit RankCalibrationXENDCG(const std::vector<std::string>& strs)
      : RankingCalibrationObjective(strs) {}

  ~RankCalibrationXENDCG() {}
